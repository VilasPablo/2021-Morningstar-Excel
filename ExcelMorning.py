...
class ExcelMorning():
    ...
    def get_holding (self, isin_fund, inception_date, obsolete_date, asset_id, holding_type='ALL',data_type = 'WEIGHT', 
        ...
        while end_date < obsolete_date:
            ...
            self.hold_info.loc[(serie_code, start_date, end_date),:] = [inception_date, obsolete_date,
    ...
    # the first attribute can not be empety
    ...
    def wait_processing(self):
        ...
        while df.index.name=='Processing...':
            ...
            df = self.sheet.range(self.sheet.range('A1'), self.sheet.range('B1').expand()).options(pd.DataFrame).value 
        time.sleep(0.75)
#         while m.locateOnScreen('processing.png', confidence=0.80)!=None:
#             time.sleep(0.1)

    def is_limit(self):
        if m.locateOnScreen('morning_limit.png', confidence=0.85)!=None:
...
def morning_singin():
    m.click(752,47)#morning
    time.sleep(1)
    m.click(35,91)#profile
    time.sleep(1)
    m.click(64,199)#direct
    time.sleep(2)
    m.press('enter')#log in
    time.sleep(5)
